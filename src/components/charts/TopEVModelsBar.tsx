...
export default function TopEVModelsBar() {
    const { data } = useSummaryStats();
    const modelData = data?.top_ev_models || [];
        ...
        <div className="h-[300px] w-full mt-4">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={modelData} margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                    ...
                    <YAxis type="category" dataKey="model" axisLine={false} tickLine={false} tick={{ fill: '#888', fontSize: 13 }} width={120} />
                    <Tooltip
                    ...
                    />
                    <Bar dataKey="count" fill="#8b5cf6" radius={[0, 4, 4, 0]} name="Registrations" barSize={24} />
                </BarChart>
            </ResponsiveContainer>
        </div>
    );
